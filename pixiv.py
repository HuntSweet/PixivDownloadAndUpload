...
# -*- encoding: utf-8 -*-
# Author:  MoeClub.org
# Modified: Huahai.club

from urllib import request
import threading
import queue,requests,json,mimetypes
import re
import os
...
class pixiv:
    ...
    def get_data_by_item(self, item):
        ...
        for data_url in data[0]:
            ...
            name = str("{}_{}").format(str(data[2]), str(str(data_url).rsplit('/', 1)[-1]))
            self.write(name, image.read())
            folder = os.path.join(self.root, self.folder)
            file = os.path.join(folder, str(name))
            try:
                self.upload(self.formatSource(file))
                print(file + "上传图床成功\n")
            except:
                print(file+"上传失败\n")
                pass
    ...
    def upload(self,files):
        ...
        APIKey = "图床api"  
        format = "json"
        url = "http://图床域名/api/1/upload/?key=" + APIKey + "&format=" + format  # 图床地址
